...
export async function render(container) {
  ...
  if (coordsBuildings.length > 0) {
    await Utils.setupBuildingMap(document.getElementById('districtMap'), coordsBuildings, {
      popupBuilder: (b, hash) => `<div class="map-popup"><div class="map-popup-header"><strong>🏛️ ${b.n}</strong></div><div class="map-popup-body"><div class="map-popup-info"><span class="map-popup-era">📅 ${b.e || ''}</span></div><a href="${hash(b)}" class="map-popup-link">查看详情 →</a></div></div>`
    });
  }
}
